...
MolybdenumDetectorConstruction::MolybdenumDetectorConstruction() {
    ...
    // World
    world_position =
        G4ThreeVector(world_position_x, world_position_y, world_position_z);

    // Target body rear part
    target_body_rear_part_position = G4ThreeVector(
        target_body_rear_part_position_x, target_body_rear_part_position_y,
    ...
    water_cooler_hollow_translation = G4ThreeVector(
        water_cooler_hollow_translation_x, water_cooler_hollow_translation_y,
        water_cooler_hollow_translation_z);
    water_cooler_hollow_transform = nullptr;

    water_cooler_position =
        G4ThreeVector(water_cooler_position_x, water_cooler_position_y,
                      water_cooler_position_z);

    // Target body front part
    target_body_front_part_hollow_rotation = new G4RotationMatrix();
    target_body_front_part_hollow_translation =
        G4ThreeVector(target_body_front_part_hollow_translation_x,
                      target_body_front_part_hollow_translation_y,
    ...
    target_body_front_part_position = G4ThreeVector(
        target_body_front_part_position_x, target_body_front_part_position_y,
        target_body_front_part_position_z);

    // Copper holder
    copper_holder_hollow_translation = G4ThreeVector(
        copper_holder_hollow_translation_x, copper_holder_hollow_translation_y,
        copper_holder_hollow_translation_z);
    copper_holder_hollow_rotation = new G4RotationMatrix();
    copper_holder_hollow_transform = nullptr;

    copper_holder_position =
        G4ThreeVector(copper_holder_position_x, copper_holder_position_y,
                      copper_holder_position_z);

    // Molybdenum target tablet
    molybdenum100_tablet_position = G4ThreeVector(
    ...
    vacuum_window_position =
        G4ThreeVector(vacuum_window_position_x, vacuum_window_position_y,
                      vacuum_window_position_z);

    // Vacuum cubic space
    vacuum_space_position =
        G4ThreeVector(vacuum_space_position_x, vacuum_space_position_y,
                      vacuum_space_position_z);
}
...
void MolybdenumDetectorConstruction::DefineMaterials() {
    ...
    aluminium_material = nist_manager->FindOrBuildMaterial("G4_ALUMINIUM");

    // ---- copper for target holder ----
    copper_material = nist_manager->FindOrBuildMaterial("G4_COPPER");

    // ---- helium gas ----
    helium_material = new G4Material("helium_gas", helium_density, 1, kStateGas,
                                     helium_temperature, helium_pressure);
    helium_material->AddElement(nist_manager->FindOrBuildElement("He"), 1);

    // ---- cooling water ----
    water_material = new G4Material("water", water_density, 2, kStateLiquid,
                                    water_temperature, water_pressure);
    ...
    // ---- molybdenum-100 ----
    auto *molybdenum100 = new G4Isotope(
        "100Mo", 42, 100, molybdenum100_atomic_mass); // 1 g/mole == 1 Da
    auto *molybdenum98 =
        new G4Isotope("98Mo", 42, 98, molybdenum98_atomic_mass);
    auto *molybdenum97 =
        new G4Isotope("97Mo", 42, 97, molybdenum97_atomic_mass);
    auto *molybdenum96 =
        new G4Isotope("96Mo", 42, 96, molybdenum96_atomic_mass);
    auto *molybdenum95 =
        new G4Isotope("95Mo", 42, 95, molybdenum95_atomic_mass);
    auto *molybdenum94 =
        new G4Isotope("94Mo", 42, 94, molybdenum94_atomic_mass);
    auto *molybdenum92 =
        new G4Isotope("92Mo", 42, 92, molybdenum92_atomic_mass);

    auto *molybdenum100_enriched =
        new G4Element("100Mo_enriched", "100Mo_92_2", 7);
    ...
    molybdenum100_material = new G4Material(
        "100Mo2C", molybdenum100_enriched_density, 1, kStateSolid);
    molybdenum100_material->AddElement(molybdenum100_enriched, 1);

    vacuum_material = new G4Material("vacuum", 1.e-25 * g / cm3, air_material,
...
void MolybdenumDetectorConstruction::BuildWorld() {
    ...
    logical_world =
        new G4LogicalVolume(solid_world, helium_material, "solid_world");
    auto *world_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> world_colors =
        convert_rgba_to_percents(std::vector{84., 150., 255., 0.2});
    world_vis_attr->SetColour(world_colors[0], world_colors[1], world_colors[2],
                              world_colors[3]);
    logical_world->SetVisAttributes(world_vis_attr);
    physical_world =
        new G4PVPlacement(nullptr, world_position, logical_world,
                          "physical_world", nullptr, false, 0, check_overlaps);
}
...
void MolybdenumDetectorConstruction::BuildTargetBodyRearPart() {
    ...
    solid_target_body_rear_part = new G4Tubs(
        "solid_target_body_rear_part", target_body_rear_part_radius_inner,
    ...
    logical_target_body_rear_part =
        new G4LogicalVolume(solid_target_body_rear_part, aluminium_material,
                            "logical_target_body_rear_part");
    ...
    target_body_rear_part_vis_attr->SetColour(
        target_body_rear_part_colours[0], target_body_rear_part_colours[1],
        target_body_rear_part_colours[2], target_body_rear_part_colours[3]);
    logical_target_body_rear_part->SetVisAttributes(
        target_body_rear_part_vis_attr);

    // A physical volume for the target body rear part
    physical_target_body_rear_part = new G4PVPlacement(
        nullptr, target_body_rear_part_position, logical_target_body_rear_part,
        "physical_target_body_rear_part", logical_world, false, 0,
        check_overlaps);
}
...
void MolybdenumDetectorConstruction::BuildWaterCooler() {
    ...
    solid_water_cooler_alum =
        new G4Tubs("solid_water_cooler_alum", water_cooler_alum_radius_inner,
                   water_cooler_alum_radius_outer, water_cooler_half_length,
                   water_cooler_phi_start, water_cooler_phi_delta);
    // A pipe for water: a tube where the water flows through
    solid_water_cooler_hollow = new G4Tubs(
        "solid_water_cooler_hollow", water_cooler_hollow_radius_inner,
        water_cooler_hollow_radius_outer, water_cooler_hollow_half_length,
        water_cooler_hollow_phi_start, water_cooler_hollow_phi_delta);

    water_cooler_hollow_rotation->rotateX(std::numbers::pi / 2 * rad);
    water_cooler_hollow_transform = new G4Transform3D(
        *water_cooler_hollow_rotation, water_cooler_hollow_translation);
    // An actual placeholder for the water
    solid_water_holder = new G4SubtractionSolid(
        "solid_water_holder", solid_water_cooler_alum,
        solid_water_cooler_hollow, *water_cooler_hollow_transform);

    logical_water_cooler_alum = new G4LogicalVolume(
        solid_water_holder, aluminium_material, "logical_water_cooler_alum");

    auto *water_cooler_alum_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> water_cooler_alum_colours =
        convert_rgba_to_percents(std::vector{165., 169., 180., 0.7});
    water_cooler_alum_vis_attr->SetColour(
        water_cooler_alum_colours[0], water_cooler_alum_colours[1],
    ...
    physical_water_cooler_alum = new G4PVPlacement(
        nullptr, water_cooler_position, logical_water_cooler_alum,
        "physical_water_cooler_alum", logical_world, false, 0, check_overlaps);

    // Now I have to create the water itself
    solid_water_flow = new G4SubtractionSolid(
        "solid_water_flow", solid_water_cooler_alum, solid_water_holder);

    logical_water_flow = new G4LogicalVolume(solid_water_flow, water_material,
                                             "logical_water_flow");
    auto *water_flow_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> water_flow_colours =
        convert_rgba_to_percents(std::vector{0., 94., 255., .2});
    water_flow_vis_attr->SetColour(water_flow_colours[0], water_flow_colours[1],
                                   water_flow_colours[2],
                                   water_flow_colours[3]);
    logical_water_flow->SetVisAttributes(water_flow_vis_attr);
    // A physical volume for the water flow
    physical_water_flow = new G4PVPlacement(
        nullptr, water_cooler_position, logical_water_flow,
        "physical_water_flow", logical_world, false, 0, check_overlaps);
}
...
void MolybdenumDetectorConstruction::BuildTargetBodyFrontPart() {
    ...
    solid_target_body_front_part_baffle = new G4Tubs(
        "solid_target_body_front_part_baffle",
        target_body_front_part_baffle_radius_inner,
    ...
    solid_target_body_front_part_hollow = new G4Tubs(
        "solid_target_body_front_part_hollow",
        target_body_front_part_hollow_radius_inner,
        target_body_front_part_hollow_radius_outer,
    ...
    target_body_front_part_hollow_transform =
        new G4Transform3D(*target_body_front_part_hollow_rotation,
                          target_body_front_part_hollow_translation);
    solid_target_body_front_part = new G4UnionSolid(
        "solid_target_body_front_part", solid_target_body_front_part_baffle,
        solid_target_body_front_part_hollow,
        *target_body_front_part_hollow_transform);

    // A logical for target body front part
    logical_target_body_front_part =
        new G4LogicalVolume(solid_target_body_front_part, aluminium_material,
                            "logical_target_body_front_part");
    ...
    const std::vector<G4double> target_body_front_part_colours =
        convert_rgba_to_percents(std::vector{165., 169., 180., 0.7});
    target_body_front_part_vis_attr->SetColour(
        target_body_front_part_colours[0], target_body_front_part_colours[1],
        target_body_front_part_colours[2], target_body_front_part_colours[3]);
    logical_target_body_front_part->SetVisAttributes(
        target_body_front_part_vis_attr);

    // A physical volume for the target body front part
    physical_target_body_front_part = new G4PVPlacement(
        nullptr, target_body_front_part_position,
        logical_target_body_front_part, "physical_target_body_front_part",
        logical_world, false, 0, check_overlaps);
}

void MolybdenumDetectorConstruction::BuildCopperHolder() {
    solid_copper_holder_baffle = new G4Tubs(
        "solid_copper_holder_baffle", copper_holder_baffle_radius_inner,
        copper_holder_baffle_radius_outer, copper_holder_baffle_half_length,
        copper_holder_baffle_phi_start, copper_holder_baffle_phi_delta);

    solid_copper_holder_hollow = new G4Tubs(
        "solid_copper_holder_hollow", copper_holder_hollow_radius_inner,
        copper_holder_hollow_radius_outer, copper_holder_hollow_half_length,
        copper_holder_hollow_phi_start, copper_holder_hollow_phi_delta);

    copper_holder_hollow_rotation->rotateX(0. * deg);
    copper_holder_hollow_transform = new G4Transform3D(
        *copper_holder_hollow_rotation, copper_holder_hollow_translation);

    solid_copper_holder = new G4UnionSolid(
        "solid_copper_holder", solid_copper_holder_baffle,
        solid_copper_holder_hollow, *copper_holder_hollow_transform);

    logical_copper_holder = new G4LogicalVolume(
        solid_copper_holder, copper_material, "logical_copper_holder");

    auto *copper_holder_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> copper_holder_colors =
        convert_rgba_to_percents(std::vector{184., 115., 51., 0.8});
    copper_holder_vis_attr->SetColour(
        copper_holder_colors[0], copper_holder_colors[1],
        copper_holder_colors[2], copper_holder_colors[3]);

    logical_copper_holder->SetVisAttributes(copper_holder_vis_attr);

    physical_copper_holder = new G4PVPlacement(
        nullptr, copper_holder_position, logical_copper_holder,
        "physical_copper_holder", logical_world, false, 0, check_overlaps);
}

void MolybdenumDetectorConstruction::BuildMolybdenumTablet() {
    solid_molybdenum100_tablet = new G4Tubs(
        "solid_molybdenum100_tablet", molybdenum100_tablet_radius_inner,
        molybdenum100_tablet_radius_outer, molybdenum100_tablet_half_length,
        molybdenum100_tablet_phi_start, molybdenum100_tablet_phi_delta);

    logical_molybdenum100_tablet =
        new G4LogicalVolume(solid_molybdenum100_tablet, molybdenum100_material,
                            "logical_molybdenum100_tablet");

    auto *molybdenum100_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> molybdenum100_colours =
        convert_rgba_to_percents(std::vector{187., 187., 187., 1.});
    molybdenum100_vis_attr->SetColour(
        molybdenum100_colours[0], molybdenum100_colours[1],
        molybdenum100_colours[2], molybdenum100_colours[3]);

    logical_molybdenum100_tablet->SetVisAttributes(molybdenum100_vis_attr);

    physical_molybdenum100_tablet = new G4PVPlacement(
        nullptr, molybdenum100_tablet_position, logical_molybdenum100_tablet,
        "physical_molybdenum100_tablet", logical_world, false, 0,
        check_overlaps);
}

void MolybdenumDetectorConstruction::BuildVacuumWindow() {
    solid_vacuum_window = new G4Box("solid_vacuum_window", vacuum_window_px,
                                    vacuum_window_py, vacuum_window_pz);

    logical_vacuum_window = new G4LogicalVolume(
        solid_vacuum_window, aluminium_material, "logical_vacuum_window");
    auto *vacuum_window_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> vacuum_window_colours =
        convert_rgba_to_percents(std::vector{165., 169., 180., .6});
    vacuum_window_vis_attr->SetColour(
        vacuum_window_colours[0], vacuum_window_colours[1],
        vacuum_window_colours[2], vacuum_window_colours[3]);
    logical_vacuum_window->SetVisAttributes(vacuum_window_vis_attr);

    physical_vacuum_window = new G4PVPlacement(
        nullptr, vacuum_window_position, logical_vacuum_window,
        "physical_vacuum_window", logical_world, false, 0, check_overlaps);
}

void MolybdenumDetectorConstruction::BuildVacuumSpace() {
    solid_vacuum_space = new G4Box("solid_vacuum_space", vacuum_space_px,
                                   vacuum_space_py, vacuum_space_pz);

    logical_vacuum_space = new G4LogicalVolume(
        solid_vacuum_space, vacuum_material, "logical_vacuum_space");
    auto *vacuum_space_vis_attr = new G4VisAttributes(true);
    const std::vector<G4double> vacuum_space_colours =
        convert_rgba_to_percents(std::vector{171., 176., 224., .1});
    vacuum_space_vis_attr->SetColour(
        vacuum_space_colours[0], vacuum_space_colours[1],
        vacuum_space_colours[2], vacuum_space_colours[3]);
    logical_vacuum_space->SetVisAttributes(vacuum_space_vis_attr);

    physical_vacuum_space = new G4PVPlacement(
        nullptr, vacuum_space_position, logical_vacuum_space,
