...
void MolybdenumHadronInelasticPhysics::ConstructProcess() {
    ...
    auto* qmd = new G4IonQMDPhysics();
    // Proton inelastic process
    G4HadronInelasticProcess* proton_inelastic =
        new G4HadronInelasticProcess("protonInelastic", G4Proton::ProtonDefinition());
    ...
    // proton_inclxx_cascade->SetDeExcitation(proton_pre_compound);
    ...
    G4HadronInelasticProcess* neutron_inelastic =
        new G4HadronInelasticProcess("neutronInelastic", G4Neutron::NeutronDefinition());

    // Low-energy HP model for neutrons (E < 20 MeV)
    G4NeutronHPInelastic* neutron_hp_model = new G4NeutronHPInelastic();
